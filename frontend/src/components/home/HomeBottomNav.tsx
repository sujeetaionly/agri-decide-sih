...
export type NavTab = 'home' | 'my-crops' | 'settings';
...
export const HomeBottomNav: React.FC<HomeBottomNavProps> = ({
  ...
  // 3 distinct primary bottom tabs: Home, My Crops, Settings
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center flex-1 py-1.5 px-2 rounded-2xl transition-all active:scale-95 ${
                isActive
            ...
            >
              ...
              <span className="text-xs mt-0.5 tracking-tight line-clamp-1">
                {t(item.labelKey)}
              </span>
            </button>
          );
